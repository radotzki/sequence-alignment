...
export class SequenceAlignment {
  calculate(sequenceS, sequenceT, match, mismatch, gap, {nw, sw, oa}) {
    const matrix = this.buildMatrix(sequenceS, sequenceT, match, mismatch, gap, {nw, sw, oa});
    const [matrix, finalS, finalT] = this.traceBack(sequenceS, sequenceT, matrix, {nw, sw, oa});
